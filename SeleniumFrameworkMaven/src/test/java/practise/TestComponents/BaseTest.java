...
public class BaseTest {
	...
	//properties class can read the global properties and decide run time on which browser need to run test cases
		...
		//In if condition need to change method from equalsIgnoreCase to contains as there are 2 browser name (chrome & chromeheadless)
		//so in contains as chrome word displayed it would go into it as it contains word chrome for chromeheadless
		if(browserName.contains("chrome")) {
			//Headless mode means browse will not show up the execution piece that would be handle by chrome by its engine API
			ChromeOptions options = new ChromeOptions();
			
			WebDriverManager.chromedriver().setup();
			//Need to set if condition as if one has to run in both the mode headless or by displaying chorme browser
			if(browserName.contains("headless"))
			{
			options.addArguments("headless");
			
			//Need to add once again the max condition as for headless mode its mandatory that whole page should be displayed so
			//script doesnot fail because of invisiblility of element
