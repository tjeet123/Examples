...
//This interface
public class ListenersThreadLocals extends BaseTest implements ITestListener {
	
	ExtentTest test;
	ExtentReports extent = ExtentReporterNG.getReportObject();
	//In parallel execution, sometimes test results are not displayed in report because they get overridden through another method at same time
	//There is concurrency issue: Multiple test trying to access one single variable which is keeping overridden and that is were result going for toss
	//Java one class called threadlocal,
	//This way object is now not supporting concurrency, using threadlocal u will make this object threadsafe means
	//No matter if u run concurrently each object creation have its own thread it would not interrupt the other overriding variable
	...
	public void onTestStart(ITestResult result) {
		...
		//It also pick up unique thread id, it not only set this test object it also extract the thread id of which test is executing that
		//with that threadid(Errorvalidationtest ) and this object it create map: (there are 3 place were we changed the code means replace test with extentTest.get())
	...
	public void onTestFailure(ITestResult result) {
		...
		//It failed now want to use test. : u want to know which error validation  created
